...
if (!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "N") {
	...
	elseif ($_POST["do_register"] == "Y" && $arResult["AUTH"]["new_user_registration"] == "Y") {
		...
		if ($_POST["NEW_GENERATE"] == "Y") {
		    ...
		    if (strlen($_POST["NEW_PASSWORD"]) > 0 && strlen($_POST["NEW_PASSWORD_CONFIRM"]) > 0 && $_POST["NEW_PASSWORD"] != $_POST["NEW_PASSWORD_CONFIRM"])
...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y") {
	...
	if ($arParams["USE_PREPAYMENT"] == "Y") {
		...
		$dbPaySysAction = CSalePaySystemAction::GetList(
				...
				), false, false, array("ID", "PAY_SYSTEM_ID", "PERSON_TYPE_ID", "NAME", "ACTION_FILE", "RESULT_FILE", "NEW_WINDOW", "PARAMS", "ENCODING", "LOGOTIP")
		);
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid())) {
		...
		$arDeliv = CSaleDelivery::GetByID($_POST["DELIVERY_ID"]);
		if (empty($arDeliv)) {
		    $arResult["ERROR"][] = GetMessage("PL_DELIVERY_NO_SELECTED");
		}
	    }

	    /* if (!$APPLICATION->CaptchaCheckCode($_POST['captcha_word'], $_POST['captcha_sid'])) {
	      $arResult["ERROR"][] = GetMessage("PL_CAPTHA_NO_SELECTED");
	      } */
	    ...
	    $dbOrderProps = CSaleOrderProps::GetList(
			    array("SORT" => "ASC"), $arFilter, false, false, array("ID", "NAME", "TYPE", "IS_LOCATION", "IS_LOCATION4TAX", "IS_PROFILE_NAME", "IS_PAYER", "IS_EMAIL", "REQUIED", "SORT", "IS_ZIP", "CODE", "MULTIPLE")
	    );
	...
	foreach ($arResult["BASKET_ITEMS"] as &$arResultItem) {
		...
		foreach ($arCustomSelectFields as $field) {
		    ...
		    if ((!isset($arResultItem[$fieldVal]) || (isset($arResultItem[$fieldVal]) && strlen($arResultItem[$fieldVal]) == 0)) && (isset($arProductData[$parentId][$fieldVal]) && !empty($arProductData[$parentId][$fieldVal]))) { // can be array or string
		...
		if ($arImage) {
		    $arFileTmp = CFile::ResizeImageGet(
				    $arImage, array("width" => "110", "height" => "110"), BX_RESIZE_IMAGE_PROPORTIONAL, true
		    );
	...
	if (IntVal($arUserResult["DELIVERY_LOCATION"]) > 0) {
		...
		if (IntVal($arUserResult["PAY_SYSTEM_ID"]) <= 0) {
		    ...
		    $dbPaySystem = CSalePaySystem::GetList(
				    array("SORT" => "ASC", "PSA_NAME" => "ASC"), $arFilter
		    );
	    ...
	    $dbList = CCatalogStore::GetList(
			    array("SORT" => "DESC", "ID" => "DESC"), array("ACTIVE" => "Y", "ID" => $arStoreId, "ISSUING_CENTER" => "Y", "+SITE_ID" => SITE_ID), false, false, array("ID", "TITLE", "ADDRESS", "DESCRIPTION", "IMAGE_ID", "PHONE", "SCHEDULE", "GPS_N", "GPS_S", "ISSUING_CENTER", "SITE_ID")
	    );
		...
		if (count($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) <= 0 || in_array($arDelivery["ID"], $arP2D[$arUserResult["PAY_SYSTEM_ID"]])) {
		    ...
		    if ((IntVal($arUserResult["DELIVERY_ID"]) == IntVal($arDelivery["ID"])) || (strlen($arUserResult["DELIVERY_ID"]) <= 0 && $bFirst)) {
	...
	while ($arPaySystem = $dbPaySystem->Fetch()) {
		...
		if (!CSalePaySystemsHelper::checkPSCompability(
				$arPaySystem["PSA_ACTION_FILE"], $arOrder, $arResult["ORDER_PRICE"], $arResult["DELIVERY_PRICE"], $arUserResult["DELIVERY_LOCATION"]
			)) {
		    continue;
		}
		...
		$arResult["PAY_SYSTEM"][$arPaySystem["ID"]]["PRICE"] = CSalePaySystemsHelper::getPSPrice(
				$arPaySystem, $arResult["ORDER_PRICE"], $arResult["DELIVERY_PRICE"], $arUserResult["DELIVERY_LOCATION"]);
	...
	$arOrderDat = CSaleOrder::DoCalculateOrder(
			SITE_ID, $USER->GetID(), $arResult["BASKET_ITEMS"], $arUserResult['PERSON_TYPE_ID'], $arUserResult["ORDER_PROP"], $arUserResult["DELIVERY_ID"], $arUserResult["PAY_SYSTEM_ID"], array(), $arErrors, $arWarnings
	);
	...
	if ($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"])) {
		...
		if (empty($arResult["ERROR"])) {
			...
			$withdrawSum = CSaleUserAccount::Withdraw(
					$USER->GetID(), $orderTotalSum, $arResult["BASE_LANG_CURRENCY"], $arResult["ORDER_ID"]
			);
			...
			if ($psPreAction && $psPreAction->IsAction()) {
			    ...
			    $dbBasketItems = CSaleBasket::GetList(
					...
					"ORDER_ID" => $arResult["ORDER_ID"]
					    ), false, false, array("ID", "QUANTITY", "PRICE", "WEIGHT", "NAME", "CURRENCY", "PRODUCT_ID", "DETAIL_PAGE_URL")
			    );
		...
		if (empty($arResult["ERROR"])) {
		    ...
		    $arBasketList = array();
		    $dbBasketItems = CSaleBasket::GetList(
				    array("ID" => "ASC"), array("ORDER_ID" => $arResult["ORDER_ID"]), false, false, array("ID", "PRODUCT_ID", "NAME", "QUANTITY", "PRICE", "CURRENCY", "TYPE", "SET_PARENT_ID")
		    );
		    while ($arItem = $dbBasketItems->Fetch()) {
			if (CSaleBasketHelper::isSetItem($arItem))
			    continue;
	...
	if ($arOrder && $arOrder["USER_ID"] == IntVal($USER->GetID())) {
	    if (IntVal($arOrder["PAY_SYSTEM_ID"]) > 0 && ($arOrder["PAYED"] != "Y" || DoubleVal($arOrder["SUM_PAID"]) <= 0 )) {
		$dbPaySysAction = CSalePaySystemAction::GetList(
				array(), array(
			    "PAY_SYSTEM_ID" => $arOrder["PAY_SYSTEM_ID"],
			    "PERSON_TYPE_ID" => $arOrder["PERSON_TYPE_ID"]
				), false, false, array("NAME", "ACTION_FILE", "NEW_WINDOW", "PARAMS", "ENCODING", "LOGOTIP")
		);
		if ($arPaySysAction = $dbPaySysAction->Fetch()) {
